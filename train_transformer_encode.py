...
from model import BikeShareTransformer,BikeShaareTransformerall
import numpy as np
...
outputpath='/home/dzhang/wudan/machine_learning/LSTM/output/model.pt'
input_size = 12
d_model = 128
nhead = 4
input_length = 96
...
def train(outputpath):
    ...
    for epoch in range(epochs):
        ...
        for i, (data, label) in enumerate(test_dataloder):
            ...
            Tmae+= mean_absolute_error(np.exp(pre.cpu().detach().numpy())-1, np.exp(y.cpu().detach().numpy())-1)
            Tmse+=mean_squared_error(np.exp(pre.cpu().detach().numpy())-1, np.exp(y.cpu().detach().numpy())-1)
...
mae_min_list=[]
for k in range(5):
    outputpath='/home/dzhang/wudan/machine_learning/LSTM/output/model_trans_encode_{output_length}_{k}.pt'
